...
maxprep_batting_team <- function(url) {
  ...
  x_team$Team <- filter(baseball_var_lu_table, stats_url == url) %>% .$school
  x_team$city_state <- filter(baseball_var_lu_table, stats_url == url) %>% .$city_state
  x_team <- select(x_team, Team, city_state, everything())
  x_team
}
